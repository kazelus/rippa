...
export default function EditModelPage({
  ...
  const fetchModel = async () => {
    try {
      ...
      try {
        const accRes = await fetch(`/api/admin/accessories?modelId=${modelId}`);
        if (accRes.ok) {
          const accData = await accRes.json();
          setLinkedAccessoryIds(accData.map((a: any) => a.id));
        }
      } catch (err) {
        console.error("Error fetching accessories:", err);
      }

      // Load all models for accessories picker
      try {
        ...
        console.error("Error fetching models for accessories:", err);
      }
    } catch (err: any) {
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      const { url } = await uploadFileDirectly(file, true);
