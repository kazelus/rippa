...
export default function ParametersAdminPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Usuń parametr? To działanie jest nieodwracalne.")) return;
    try {
      ...
      <header className="bg-[#1a1f2e] border-b border-[#1b3caf]/30">
        <div className="w-full px-4 sm:px-6 lg:px-8 py-6 flex items-center justify-between">
          <h1 className="text-2xl font-bold text-white">
            Zarządzaj parametrami technicznymi
          </h1>
        </div>
      </header>
        ...
        <div className="bg-[#1a1f2e] border border-[#1b3caf]/30 rounded-xl p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {formOpen ? "Anuluj" : "Dodaj nowy parametr"}
              </button>
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    className="w-full px-3 py-2 rounded bg-[#1a1f2e] text-white border border-[#1b3caf]/20"
                    placeholder='e.g. ["Mała","Średnia","Duża"]'
                  />
                </div>
                <div>
                  <label className="text-sm text-white block mb-1">
          ...
          <div>
            {loading ? (
              ...
              <div className="space-y-2">
                {parameters.length === 0 && (
                  <p className="text-white">Brak zdefiniowanych parametrów.</p>
                )}
                {parameters.map((p) => (
                  <div
                    key={p.id}
